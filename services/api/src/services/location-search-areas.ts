...
const INSERT_POSTCODE_AREAS = `
  ...
  INSERT INTO {destination} (${AREA_COLUMNS})
  SELECT
    'postcode:' || country_code || ':' || postcode_match AS area_key,
  ...
  GROUP BY country_code, postcode_match
`;
...
const INSERT_STREET_AREAS = `
  ...
  INSERT INTO {destination} (${AREA_COLUMNS})
  SELECT
    'street:' || country_code || ':' || street_token || ':city=' || city_token AS area_key,
    'street' AS area_kind,
  ...
  await db.transaction(async (tx) => {
    ...
    await tx.execute(sql.raw(`
      WITH affected_area_keys AS (
        ...
        UNION
        SELECT DISTINCT 'postcode:' || country_code || ':' || postcode_match AS area_key
        FROM affected_location_search_area_keys
        WHERE postcode_match IS NOT NULL
        UNION
        ...
        UNION
        SELECT DISTINCT 'street:' || country_code || ':' || street_token || ':city=' || city_token AS area_key
        FROM affected_location_search_area_keys
    ...
    await tx.execute(sql.raw(`
      ...
      WHERE p.status = 'active'
      GROUP BY p.country_code, target.postcode_match
      ${TARGETED_AREA_UPSERT}
    `));

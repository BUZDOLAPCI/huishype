...
export const officialValuationSourceStates = pgTable(
  'official_valuation_source_states',
  {
    ...
    adaptiveRequestsPerMinute: integer('adaptive_requests_per_minute').notNull().default(60),
    adaptiveConcurrency: integer('adaptive_concurrency').notNull().default(1),
    throttleUntil: timestamp('throttle_until', { withTimezone: true }),
    cleanSuccessCount: integer('clean_success_count').notNull().default(0),
    cleanConcurrencyWindowCount: integer('clean_concurrency_window_count').notNull().default(0),
    recentRateLimitCount: integer('recent_rate_limit_count').notNull().default(0),
    lastObservedStatus: integer('last_observed_status'),
    lastObservedRetryAfter: text('last_observed_retry_after'),
    lastObservedRateLimitReset: text('last_observed_rate_limit_reset'),

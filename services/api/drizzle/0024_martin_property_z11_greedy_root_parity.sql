...
AS $$
  ...
  legacy_root_seed_candidates AS MATERIALIZED (
    SELECT c.*
  ...
  z11_primary_orphans AS MATERIALIZED (
    SELECT c.*
    FROM active_candidates c
    WHERE z = 11
      AND NOT EXISTS (
        SELECT 1
        FROM legacy_seed_matches matched
        WHERE matched.property_id = c.property_id
      )
  ),
  z11_secondary_root_seed_candidates AS MATERIALIZED (
    SELECT c.*
    FROM z11_primary_orphans c
    WHERE NOT EXISTS (
      SELECT 1
      FROM z11_primary_orphans higher
      WHERE higher.priority_rank < c.priority_rank
        AND higher.cell_x BETWEEN c.cell_x - 1 AND c.cell_x + 1
        AND higher.cell_y BETWEEN c.cell_y - 1 AND c.cell_y + 1
        AND higher.world_x_units BETWEEN c.world_x_units - 208.0 AND c.world_x_units + 208.0
  ...
  z11_secondary_seed_matches AS (
    SELECT DISTINCT ON (f.property_id)
      f.property_id,
      s.property_id AS seed_property_id
    FROM z11_primary_orphans f
    INNER JOIN z11_secondary_root_seed_candidates s
      ON s.cell_x BETWEEN f.cell_x - 1 AND f.cell_x + 1
      AND s.cell_y BETWEEN f.cell_y - 1 AND f.cell_y + 1
      AND s.world_x_units BETWEEN f.world_x_units - 208.0 AND f.world_x_units + 208.0
      AND s.world_y_units BETWEEN f.world_y_units - 208.0 AND f.world_y_units + 208.0
  ...
  z11_secondary_orphans AS MATERIALIZED (
    SELECT c.*
    FROM z11_primary_orphans c
    WHERE NOT EXISTS (
      SELECT 1
      FROM z11_secondary_seed_matches matched
      WHERE matched.property_id = c.property_id
    )
  ),
  z11_tertiary_root_seed_candidates AS MATERIALIZED (
    SELECT c.*
    FROM z11_secondary_orphans c
    WHERE NOT EXISTS (
      SELECT 1
      FROM z11_secondary_orphans higher
      WHERE higher.priority_rank < c.priority_rank
        AND higher.cell_x BETWEEN c.cell_x - 1 AND c.cell_x + 1
        AND higher.cell_y BETWEEN c.cell_y - 1 AND c.cell_y + 1
        AND higher.world_x_units BETWEEN c.world_x_units - 208.0 AND c.world_x_units + 208.0
  ...
  z11_tertiary_seed_matches AS (
    SELECT DISTINCT ON (f.property_id)
      f.property_id,
      s.property_id AS seed_property_id
    FROM z11_secondary_orphans f
    INNER JOIN z11_tertiary_root_seed_candidates s
      ON s.cell_x BETWEEN f.cell_x - 1 AND f.cell_x + 1
      AND s.cell_y BETWEEN f.cell_y - 1 AND f.cell_y + 1
      AND s.world_x_units BETWEEN f.world_x_units - 208.0 AND f.world_x_units + 208.0
      AND s.world_y_units BETWEEN f.world_y_units - 208.0 AND f.world_y_units + 208.0
  ...
  z11_seed_matches AS (
    SELECT property_id, seed_property_id
    FROM legacy_seed_matches
    UNION ALL
    SELECT property_id, seed_property_id
    FROM z11_secondary_seed_matches
    UNION ALL
    SELECT property_id, seed_property_id
    FROM z11_tertiary_seed_matches
  ),
  seed_matches AS (
    SELECT property_id, seed_property_id
    FROM z11_seed_matches
    WHERE z = 11
    UNION ALL
    SELECT property_id, seed_property_id
    FROM legacy_seed_matches

...
Create a new folder under `reference-expectations/` with a descriptive kebab-case name:

```
reference-expectations/
└── your-expectation-name/
    ├── expectation.md
    └── reference-image.png (optional)
```

**Naming conventions:**
- Use kebab-case: `price-guess-slider-ui`, `map-visuals-close-up`
...
| "Work on reference expectations X and Y" | Process multiple named expectations |
...
```bash
mkdir reference-expectations/dark-mode-toggle
```

### 2. Write expectation.md
...
> "Work on reference expectation dark-mode-toggle"

...
- Social state: writes that affect public map social scoring/counts, including comments, property likes/reactions, price guesses, and any property activity aggregation used by `property-grouping`.
...
- after social write paths commit changes that affect public map scoring or counts: comments, property reactions/likes, price guesses, and any property activity aggregation write used by `property-grouping`.
...
- Same-key running work during uncancellable SQL becomes `detached-draining`, keeps the runtime slot occupied, and does not start duplicate same-key SQL.
...
- Comment, property reaction/like, price-guess, and property/import maintenance write paths request snapshot refresh after commit.

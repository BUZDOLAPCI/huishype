...
- **Per-query country filtering**: Photon 1.1.0+ supports `&countrycode=XX` (single ISO 3166-1 alpha-2 code per query) on `/api` to scope search to the user's current context. Photon 1.0.1 can return `properties.countrycode` but rejects `countrycode` on `/api`; backend adapters must either run 1.1.0+ or retry without the unsupported parameter and filter locally. Additional filters: `bbox`, `osm_tag`, `state`, `city`, `postcode`, `district`.

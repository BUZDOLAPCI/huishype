...
- Agree on the full locked contract before branching: grouped map fields, property detail/batch fields, public `MapFilters.activity`, explicit app-only `socialScope`, canonical activity payload/schema coverage for `GET /activity?scope=public`, `GET /activity?scope=following`, and `GET /users/me/activity`, optional-auth profile relationship/count fields, authenticated following viewport route shape, and the canonical notification vocabulary including `new_follower`.
...
- Finalize the public property/map transport contract:
  ...
  - property fields for `hasListing`, `hasActiveListing`, `marketState`, `latestListingStatus`, `socialScore`, `recentSocialScore`, and the required public engagement breakdowns
...
- Finalize notification canonicalization:
  ...
  - route schemas, exported OpenAPI, generated client, app rendering, mocks, fixtures, and producers must conform to that vocabulary before adding `new_follower`
...
- Route schemas, `services/api/openapi.json`, `packages/api-client/generated/api.ts`, mocks, and touched callers all agree on the locked grouped/property fields, canonical activity schema, profile relationship/count fields, follow endpoints, and notification vocabulary.
...
- Add `PUT /users/:id/follow` and `DELETE /users/:id/follow` with idempotent semantics.
...
- Canonicalize notification route schemas, exported OpenAPI, generated client usage, app rendering, mocks, fixtures, and existing producers onto the existing DB/service/shared notification vocabulary, then add `new_follower`.
- Clean up activity route internals so `GET /activity?scope=public|following` and `GET /users/me/activity` share one canonical schema, mapper, ordering, and query model.
...
- Add the authenticated following viewport endpoint only after Phase 2 stabilizes:
  ...
  - excludes `save`
  - accepts bbox plus normal market-state and price filters
...
- API integration tests for:
  ...
  - following viewport auth, bbox behavior, and qualifying activity rules
...
- Notification schemas, generated client usage, app rendering, mocks, fixtures, and producers use one canonical event vocabulary with `new_follower` added and no drift names remaining.
- The following viewport endpoint exists as an authenticated sparse overlay transport, explicitly separate from public tiles, nearby, and public filter serialization.
...
- Model `socialScope: 'all' | 'following'` as app-level map-view state separate from shared/public `MapFilters`, expose `Following` as a dedicated top-level rail toggle near the existing quick filters, serialize it only in committed app state on web, and keep it out of shared/public filter serializers, tile URLs, nearby URLs, and public cache keys.
- Fetch and render the authenticated sparse following overlay separately from the unchanged public map layers; do not add clustering, server-side grouping, or personalized public tiles in v1.
...
- App component/query/state tests for:
  ...
  - `socialScope` web URL/state serialization while proving public tile URLs, nearby URLs, and shared filter signatures remain unchanged
...
- Web E2E minimum set:
  ...
  - map path: sign in -> follow user with qualifying activity -> enable following mode -> verify the public base map still renders -> verify only qualifying followed-user overlay items render -> verify tapping an overlay item opens the correct property via overlay payload identity, not viewer-specific nearby/tile hit testing
...
- `socialScope='following'` works as app-only state driving an authenticated sparse overlay on top of unchanged public tiles, URLs, nearby behavior, and shared cache keys, with overlay click-through resolved from overlay payload identity and required analytics emitted.
...
- Property copy and badges separate listing lifecycle, social state, and recent activity without collapsing back into one-axis `Quiet/Active/Hot` language.

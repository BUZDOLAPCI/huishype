...
- Model `socialScope: 'all' | 'following'` as app-level map-view state separate from shared/public `MapFilters`, expose `Following` as a dedicated top-level rail toggle near the existing quick filters, serialize it only in private app state on web, never into browser query params, keep native parity in in-memory map-view state, and keep it out of shared/public filter serializers, tile URLs, nearby URLs, and public cache keys.
...
- App component/query/state tests for:
  ...
  - `socialScope` private web app-state serialization while proving browser query params, public tile URLs, nearby URLs, shared filter signatures, and public `activity` behavior remain unchanged
...
- `socialScope='following'` works as app-only private state driving an authenticated sparse overlay on top of unchanged public tiles, nearby behavior, browser query params, and shared cache keys, with signed-out gating, signed-in empty-state proof, native parity, overlay-fetch triggering, runtime proof that app-only `socialScope` stays orthogonal to public `activity`, public base-layer taps still using existing public property/nearby behavior, and overlay click-through resolved from overlay payload identity.

...
async function openPreviewCard(page: Page): Promise<void> {
  ...
  expect(
    await waitForPreviewCardVisible(page, 500),
    'Preview card should appear after clicking a rendered property feature'
  ).toBe(true);
}
...
async function openExpandedPanelFromPreview(page: Page): Promise<void> {
  if (await waitForPanelOpen(page, 500)) {
    return;
  }
  ...
  expect(
    await waitForPanelOpen(page, 500),
    'Property panel should open after tapping the preview card'
  ).toBe(true);
}

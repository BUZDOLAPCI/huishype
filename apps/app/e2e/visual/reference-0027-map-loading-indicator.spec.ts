...
test.describe(`Reference Expectation: ${EXPECTATION_NAME}`, () => {
  ...
  test('verify loading indicator appears during initial map load', async ({ page }) => {
    // Create a promise to track the loading indicator visibility
    let loadingIndicatorWasVisible = false;

    // Navigate to the app - don't wait for full network idle so we can catch the loading state
    await page.goto('/', { waitUntil: 'domcontentloaded' });

    // Check for loading indicator within first moments of page load
    // We need to check quickly before the map finishes loading
    const checkLoadingIndicator = async (): Promise<boolean> => {
      try {
        // Check for the loading indicator by various selectors
        const loadingByTestId = page.locator('[data-testid="map-loading-indicator"]');
        const loadingByText = page.locator('text=Loading map...');
        const spinnerElement = page.locator('.border-blue-500.border-t-transparent, [class*="animate-spin"]');

        const isVisibleByTestId = await loadingByTestId.isVisible({ timeout: 100 }).catch(() => false);
        const isVisibleByText = await loadingByText.isVisible({ timeout: 100 }).catch(() => false);
        const isVisibleBySpinner = await spinnerElement.isVisible({ timeout: 100 }).catch(() => false);

        return isVisibleByTestId || isVisibleByText || isVisibleBySpinner;
      } catch {
        return false;
      }
    };

    // Try to capture the loading state quickly
    // The loading indicator should be visible immediately when page loads
    for (let i = 0; i < 30; i++) {
      const isVisible = await checkLoadingIndicator();
      if (isVisible) {
        loadingIndicatorWasVisible = true;
        console.log(`Loading indicator detected at attempt ${i + 1}`);
    ...
    // If we didn't catch the loading state (map loaded too fast), that's still OK
    // The important thing is that the implementation exists
    if (!loadingIndicatorWasVisible) {
      console.log('Loading indicator was not captured - map may have loaded too quickly');
      console.log('This is acceptable behavior if the map loads fast');
    }
    ...
    // Verify map is now loaded and loading indicator is gone
    const mapView = page.locator('[data-testid="map-view"]');
    await expect(mapView).toBeVisible({ timeout: 10000 });

    // Loading indicator should NOT be visible after map loads
    ...
    console.log(`Loading indicator visible after map load: ${loadingStillVisible}`);
    expect(loadingStillVisible, 'Loading indicator should disappear once map is loaded').toBe(false);
    ...
    // Verify map canvas is visible (map fully loaded)
  ...
  test('verify loading indicator styling matches app design', async ({ page }) => {
    ...
    // Intercept style loading to slow it down artificially
    await page.route('**/tiles.openfreemap.org/**', async (route) => {
      // Delay tile loading to keep loading indicator visible
  ...
  test('capture main screenshot for visual comparison', async ({ page }) => {
    ...
    // Wait for map to fully load
    await page.waitForSelector('[data-testid="map-view"]', { timeout: 30000 });
    await page.waitForTimeout(5000); // Extra time for map rendering

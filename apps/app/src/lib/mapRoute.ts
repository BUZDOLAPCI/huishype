...
const ADDRESS_TRAILING_HOUSE_PATTERN = /^(?<street>.+?)\s+(?<house>\d[\dA-Za-z\s/-]*)$/u;
...
function isAreaLevelResult(result: ResolvedAddress): boolean {
  return !result.details.street && !result.details.houseNumber && !result.details.number;
}
...
function matchesCityRoute(
  result: ResolvedAddress,
  route: Extract<ParsedMapRoute, { kind: 'city' }>,
): boolean {
  return (
    isAreaLevelResult(result) &&
    !!result.details.city &&
    buildCanonicalCitySlug(result.details.city) === route.citySlug &&
    (result.details.countryCode?.toUpperCase() ?? route.countryCode) === route.countryCode
  );
}

function matchesPostcodeRoute(
  result: ResolvedAddress,
  route: Extract<ParsedMapRoute, { kind: 'postcode' }>,
): boolean {
  if (!isAreaLevelResult(result) || !result.details.city || !result.details.zip) {
    return false;
  }

  return (
    buildCanonicalCitySlug(result.details.city) === route.citySlug &&
    buildCanonicalPostcodeSlug(result.details.zip, route.countryCode) === route.postcodeSlug &&
    (result.details.countryCode?.toUpperCase() ?? route.countryCode) === route.countryCode
  );
}

function matchesPropertyRoute(
  result: ResolvedAddress,
  route: AddressLeafRoute,
): boolean {
  const routeInput = buildCanonicalInputFromResolvedAddress(result);
  if (!routeInput) {
    return false;
  }

  return (
    buildCanonicalCitySlug(routeInput.city) === route.citySlug &&
    buildCanonicalPostcodeSlug(routeInput.postalCode, route.countryCode) === route.postcodeSlug &&
    buildCanonicalStreetSlug(routeInput.streetName) === route.streetSlug &&
    buildCanonicalHouseSegment(
      routeInput.houseNumber,
      routeInput.houseNumberAddition,
    ) === route.houseSegment &&
    (routeInput.countryCode?.toUpperCase() ?? route.countryCode) === route.countryCode
  );
}
...
export function parseMapRoutePath(pathname: string): ParsedMapRoute {
  ...
  if (resolution.remainingSegments.length === 5) {
    const [citySlug, postcodeSlug, streetSlug, houseSegment, leaf] = resolution.remainingSegments;
...
export async function resolveMapRoute(pathname: string): Promise<ResolvedMapRoute> {
  ...
  if (parsed.kind === 'city') {
    const area = await resolvePropertyArea({
      city: slugToQueryText(parsed.citySlug),
  ...
  if (parsed.kind === 'postcode') {
    ...
    const area = await resolvePropertyArea({
      city: slugToQueryText(parsed.citySlug),
      postalCode: normalizePostalCode(parsed.postcodeSlug.toUpperCase(), parsed.countryCode),
      countryCode: parsed.countryCode,
    });

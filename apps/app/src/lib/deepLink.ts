...
export function extractAppPathFromUrl(url: string | null | undefined, depth = 0): string | null {
  if (!url || depth > 2) {
    return null;
  }
  ...
  if (isDevClientWrapper) {
    ...
    return decodedNestedUrl ? extractAppPathFromUrl(decodedNestedUrl, depth + 1) : null;
  }

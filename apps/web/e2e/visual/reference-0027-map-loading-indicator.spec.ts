...
test.describe(`Reference Expectation: ${EXPECTATION_NAME}`, () => {
  ...
  test('verify loading indicator appears during initial map load', async ({ page }) => {
    // Intercept the actual initial style fetch so the loading text stays visible
    // long enough for a reliable assertion. The map runtime now loads from the
    // API-backed `/tiles/style.json` endpoint before any tile requests are made.
    let releaseStyle: (() => void) | null = null;
    const styleBlocked = new Promise<void>((resolve) => {
    ...
    console.log('Loading text "Loading map..." is visible while style loading is blocked');
    ...
    // Release the blocked style request so the map can finish loading.
    releaseStyle!();
    ...
    console.log('Loading text hidden after map loaded');

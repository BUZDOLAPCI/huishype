...
test.describe('Save Flow', () => {
  ...
  test('property page take action section includes save action', async ({ page, request }) => {
    ...
    await expect(page.locator('button[aria-label="Like"]').first()).toBeVisible({ timeout: 10000 });
    await expect(page.locator('button[aria-label="Comment"]').first()).toBeVisible({ timeout: 10000 });
    await expect(page.locator('button[aria-label="Guess"]').first()).toBeVisible({ timeout: 10000 });
    await expect(page.locator('button[aria-label="Save"]').first()).toBeVisible({ timeout: 10000 });
    await expect(page.locator('button[aria-label="Share"]').first()).toBeVisible({ timeout: 10000 });
  });

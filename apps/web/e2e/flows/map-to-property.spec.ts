...
test.describe('Map to Property Flow', () => {
  ...
  test('click on property marker shows preview card', async ({ page }) => {
    ...
    expect(
      ...
      'Selected marker should stay in the lower-middle of the desktop map after selection'
    ).toBeGreaterThan(0.4);
    expect(markerViewportPosition!.yRatio).toBeLessThan(0.6);
  ...
  test('clicking another visible property reopens the preview after closing it', async ({ page }) => {
    await focusMapOnSeededPropertyArea(page);
    ...
    expect(secondClick.success, 'Should be able to open another preview after closing the current one').toBe(true);
    await expect(previewCard).toBeVisible({ timeout: 10000 });

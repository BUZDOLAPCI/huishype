...
test.describe('Auth Flow', () => {
  ...
  test('Auth API: browser Google login creates a session cookie envelope', async ({ request }) => {
    ...
    expect(body.session.user).toHaveProperty('createdAt');
    expect(response.headers()['set-cookie'] ?? '').toContain('HttpOnly');

    const sessionResp = await request.get(`${API_BASE_URL}/auth/session`);
    ...
    expect(sessionBody.user.id).toBe(body.session.user.id);
  });
  ...
  test('Auth API: token refresh works', async ({ request }) => {
    const user = await createTestUser(request, 'refresh');

    const refreshResp = await request.post(`${API_BASE_URL}/auth/token/refresh`, {
    ...
    expect(refreshBody.session).toHaveProperty('accessToken');
    expect(refreshBody.session).toHaveProperty('refreshToken');
    expect(refreshBody.session).toHaveProperty('expiresAt');
    expect(refreshBody.session.user.id).toBe(user.userId);
  });
  ...
  test('Auth API: browser logout clears the session cookie jar', async ({ request }) => {
    ...
    expect(loginBody.session.user).toHaveProperty('id');

    const sessionResp = await request.get(`${API_BASE_URL}/auth/session`);
    expect(sessionResp.ok()).toBe(true);
    const sessionBody = await sessionResp.json();
    expect(sessionBody.user?.id).toBe(loginBody.session.user.id);
    ...
    const postLogoutSession = await request.get(`${API_BASE_URL}/auth/session`);
    expect(postLogoutSession.ok()).toBe(true);
    const postLogoutBody = await postLogoutSession.json();
    expect(postLogoutBody).toEqual({ user: null });
  });
});

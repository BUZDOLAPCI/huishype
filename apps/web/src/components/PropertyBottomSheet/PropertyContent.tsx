...
import { View, StyleSheet } from '../../runtime/dom';
...
function PropertyContentSections({
  ...
  const guessSectionRef = useRef<HTMLDivElement | null>(null);
  const commentsSectionRef = useRef<HTMLDivElement | null>(null);

  const measureSectionOffsets = useCallback(() => {
    const stack = sectionStackRef.current;
    const guess = guessSectionRef.current;
    const comments = commentsSectionRef.current;

    if (stack && guess) {
      onGuessSectionLayout?.(guess.offsetTop);
    }
    if (stack && comments) {
      onCommentsSectionLayout?.(comments.offsetTop);
    }
  }, [onCommentsSectionLayout, onGuessSectionLayout]);

  useLayoutEffect(() => {
    measureSectionOffsets();
  }, [contentWidth, measureSectionOffsets, property?.id]);

  useEffect(() => {
    if (typeof ResizeObserver === 'undefined') {
      window.addEventListener('resize', measureSectionOffsets);
      return () => window.removeEventListener('resize', measureSectionOffsets);
    }

    const nodes = [sectionStackRef.current, guessSectionRef.current, commentsSectionRef.current].filter(Boolean) as HTMLElement[];
    if (nodes.length === 0) {
      return undefined;
    }

    const observer = new ResizeObserver(measureSectionOffsets);
    nodes.forEach((node) => observer.observe(node));
    return () => observer.disconnect();
  }, [measureSectionOffsets, property?.id]);
  ...
  return (
    <>
      <View style={styles.contentShell}>
        ...
        <View ref={sectionStackRef} style={styles.sectionStack} testID="property-content-section-stack">
          ...
          <View ref={guessSectionRef} testID="property-content-guess-section">
            <PriceGuessSection
              property={property}
          ...
          <View ref={commentsSectionRef} testID="property-content-comments-section">
            <CommentsSection
              property={property}

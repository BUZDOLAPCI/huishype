...
async function saveProperty(propertyId: string): Promise<{ saved: boolean }> {
...
async function unsaveProperty(propertyId: string): Promise<{ saved: boolean }> {
  const response = await fetch(`${API_URL}/properties/${propertyId}/save`, {
    method: 'DELETE',
    credentials: 'include',
  });
...
export function usePropertySave({
  ...
  const saveMutation = useMutation({
    mutationFn: ({ propId }: { propId: string }) => saveProperty(propId),
  ...
  const unsaveMutation = useMutation({
    mutationFn: ({ propId }: { propId: string }) => unsaveProperty(propId),
    onMutate: async ({ propId }) => {
      const key = propertyKeys.detail(propId);
      await queryClient.cancelQueries({ queryKey: key });
      const previous = queryClient.getQueryData<EnrichedProperty>(key);
  ...
  const toggleSave = useCallback(async () => {
    ...
    try {
      if (isSaved) {
        await unsaveMutation.mutateAsync({ propId: propertyId });
        return;
      }

      await saveMutation.mutateAsync({ propId: propertyId });
    } catch {

...
async function likeProperty(propertyId: string): Promise<{ liked: boolean; likeCount: number }> {
...
async function unlikeProperty(propertyId: string): Promise<{ liked: boolean; likeCount: number }> {
  const response = await fetch(`${API_URL}/properties/${propertyId}/like`, {
    method: 'DELETE',
    credentials: 'include',
  });
...
export function usePropertyLike({
  ...
  const likeMutation = useMutation({
    mutationFn: ({ propId }: { propId: string }) => likeProperty(propId),
  ...
  const unlikeMutation = useMutation({
    mutationFn: ({ propId }: { propId: string }) => unlikeProperty(propId),
    onMutate: async ({ propId }) => {
      const key = propertyKeys.detail(propId);
      await queryClient.cancelQueries({ queryKey: key });
      const previous = queryClient.getQueryData<EnrichedProperty>(key);
  ...
  const toggleLike = useCallback(() => {
    ...
    void (async () => {
      ...
      if (isLiked) {
        unlikeMutation.mutate({ propId: propertyId });
      } else {
        likeMutation.mutate({ propId: propertyId });
      }
    })();
